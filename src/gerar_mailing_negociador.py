...
from src.utils import iniciar_spinner, parar_spinner
...
logger = logging.getLogger("mailing_negociador")
if not logger.handlers:
    logger.setLevel(getattr(logging, LOG_LEVEL, logging.INFO))
    fmt = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s")
    # Log de processo por script
    fh = logging.FileHandler(os.path.join(LOG_DIR, "mailing_negociador.log"), encoding="utf-8")
...
# Função para determinar o número máximo de telefones principais
def get_max_telefones(cursor, is_principal=True):
    query = f"""
        ...
        FROM (
            ...
            WHERE 
                PesTelefoneInativo <> 1 
                AND PesTelefonePrincipal = {1 if is_principal else 0}
                AND PesTelefone <> ''
        ) AS Temp;
    """
...
# Query principal sem NOT EXISTS
query_dynamic = f"""
WITH Inadimplentes AS (
    SELECT DISTINCT
        ...
        END AS FAIXA_AGING,
        dbo.RetornaNomeUsuario(CP.CaOperadorProprietarioID) AS 'NEGOCIADOR'
    FROM Movimentacoes
        INNER JOIN Pessoas ON MoInadimplentesID = Pessoas_ID
        OUTER APPLY (
            SELECT TOP 1 CaOperadorProprietarioID 
            FROM CampanhasPessoas CP 
            WHERE MoInadimplentesID = CP.CaPessoasID 
              AND MoCampanhasID = CP.CaCampanhasId
        ) CP
    WHERE 
    ...
    GROUP BY 
        ...
        MoInadimplentesID,
        dbo.RetornaNomeUsuario(CP.CaOperadorProprietarioID)
),
...
# Executar a query principal com spinner visível
...
print(f"Extração concluída: registros={len(df)} negociadores={df['NEGOCIADOR'].nunique()} campanhas={df['CAMPANHA'].nunique()}")
logger.info(
    "Extração concluída: registros=%s negociadores=%s campanhas=%s",
    len(df), df['NEGOCIADOR'].nunique(), df['CAMPANHA'].nunique()
)
...
print(f"Após filtro RO: registros={len(df)} negociadores={df['NEGOCIADOR'].nunique()} campanhas={df['CAMPANHA'].nunique()}")
logger.info(
    "Após filtro RO: registros=%s negociadores=%s campanhas=%s",
    len(df), df['NEGOCIADOR'].nunique(), df['CAMPANHA'].nunique()
)

# Limpeza de telefones
telefone_cols = [col for col in df.columns if col.startswith('telefone_principal') or col.startswith('telefone_nao_principal')]

def limpar_telefone(telefone):
    if pd.isnull(telefone):
        return ""
    telefone = re.sub(r'\D', '', str(telefone))
    if "NA" in telefone.upper():
        return ""
...
base_dir = os.path.join("data", "negociadores")
os.makedirs(base_dir, exist_ok=True)

print(f"Gerando arquivos CSV por negociador/campanha: grupos={df.groupby(['NEGOCIADOR','CAMPANHA']).ngroups}")
logger.info("Gerando arquivos CSV por negociador/campanha: grupos=%s", df.groupby(['NEGOCIADOR','CAMPANHA']).ngroups)
...
logger.info("Arquivos de mailing gerados com sucesso!")
